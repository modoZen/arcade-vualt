...
Encontrados en revisión manual después de marcar el spec como Implementado, ya corregidos en la misma rama:

- **Bug: `username` no se resolvía para logins OAuth (Google/GitHub).** El Data model original definía `username` solo como `user?.user_metadata?.display_name ?? null`, campo que únicamente puebla nuestro propio signup por email/password. Google/GitHub nunca setean `display_name` — setean `full_name`/`name`/`user_name`. Como consecuencia, cualquier usuario logueado por OAuth tenía sesión real pero `username: null`, y el Nav/HUD de los juegos lo mostraban como invitado. Fix en `app/context/UserContext.tsx`: `username` ahora cae en cascada por `display_name ?? full_name ?? name ?? user_name ?? null`. No requirió tocar ningún consumidor (`Nav.tsx` ni las 6 páginas `jugar`), ya manejaban `username` nulo correctamente.
- **Mejora de UI: cuenta logueada en `Nav.tsx`.** El spec original solo pedía "usa `username`/`avatarUrl` de `useUser`", sin especificar cómo. Implementado como: avatar circular (`avatarUrl`, con fallback a un monograma con la inicial si no hay foto o si la imagen falla al cargar vía `onError`) + nombre separado + botón explícito "Cerrar Sesión" (antes el nombre mismo era el botón, sin acción clara). Aplicado tanto en el nav de escritorio como en el panel móvil, que antes no ofrecía forma de cerrar sesión.
